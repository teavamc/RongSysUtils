...
//	private static char symbol1 = 13;
	...
	public  Port(String portName) {
		try {
			...
			try {
				...
				System.gc();
				logger.error("出错日志  记录:" + e.getMessage().toString() + "  信息:COMM口打开失败" );
			}
	...
	public  boolean chakanPort() {
		try {
			...
			System.gc();
			logger.error("出错日志  记录:" + ex.getMessage().toString() + "  信息:SIM不存在" );
			return false;
		}
	}
	/**
	...
	public void close() {
		try {
			...
			logger.error("出错日志  记录:" + e.getMessage().toString() + "  信息:COM口关闭失败" );
		}
		serialPort.close();	
		System.gc();
		setIsused(false);		
	...
	public String sendAT(String atcommand) throws java.rmi.RemoteException {
		...
		try {
			...
			System.gc();
			logger.error("出错日志  记录:" + e.getMessage().toString() + "  信息:AT指令发送失败" );
